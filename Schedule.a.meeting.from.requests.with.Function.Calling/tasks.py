...
async def _execute_schedule(
    ...
    """
    Execute the calendar scheduling task.

    Uses TaskAsyncSession to ensure that the database session, and its async loop is
    scoped / remain bind to this celery task event loop to prevent error:
    "attached to a different loop".
    """

    # Use NullPool so connections are never pooled/shared across isolated event loops
    ...
    try:
        ...
        # 2. Persist record using task-scoped session
        ...
        # 3. Clean up the engine bindings when the task finishes
